...
await fs.writeFile("feed.json", JSON.stringify({ version: "https://jsonfeed.org/version/1.1", title: "ЗакупВарта — активні закупівлі Prozorro", home_page_url: site, feed_url: `${site}/feed.json`, language: "uk-UA", items: tenders.map((tender) => ({ id: `${tender.id}:${tender.dateModified || ""}`, url: `https://prozorro.gov.ua/tender/${encodeURIComponent(tender.tenderID || tender.id)}`, title: tender.title || tender.tenderID, content_text: `${tender.procuringEntity?.name || "Замовник не вказаний"}; ${tender.value?.amount || 0} ${tender.value?.currency || "UAH"}`, date_modified: tender.dateModified || now.toISOString() })) }, null, 2), "utf8");
